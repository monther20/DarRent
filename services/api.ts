...
export const contractsApi = {
  ...
  getByProperty: async (propertyId: string): Promise<RentalContract[]> => {
    await delay(400);
    return mockRentalContracts.filter((c) => c.propertyId === propertyId).map((c) => ({ ...c }));
  },

  getByRenter: async (renterId: string): Promise<RentalContract[]> => {
    await delay(500);
    return mockRentalContracts.filter((c) => c.renterId === renterId).map((c) => ({ ...c }));
  },
  ...
  getActiveContracts: async (): Promise<RentalContract[]> => {
    await delay(400);
    return mockRentalContracts.filter((c) => c.status === 'active').map((c) => ({ ...c }));
  },

  createContract: async (
    contractData: Omit<RentalContract, 'id' | 'createdAt'>,
